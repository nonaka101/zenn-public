...
本記事では `Date` 型を使いますが、**下記の機能を実装できれば、多言語でも同じことができます**。
...
月日の妥当性チェックでは、月や日の範囲について検証しました。しかしそれでも、「2月31日」や「1999年2月29日（閏年でない）」といった存在しない数値の可能性が残っています。
...
次に、`age` を使って**現年齢になった加齢タイミング**を求めます。月日については生年月日のデータをそのまま使いますが、年に関しては場合分けになります。今年の誕生日を越えている場合は**その年**に、そうでなければ**前年**となります。

ここでは、**誕生年に満年齢を足す**ことで算出しています。
...
ここでは誕生日は **2000年8月23日**、今年は2024年（閏年）としています。誕生日前後を今日とした場合の計算結果は、下記のとおりとなります。
