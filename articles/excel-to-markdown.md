...
「`markdown` から表形式データへの変換」は、現在作成中となっております。
...
ここでは、**別の文字列に置き換える**ことで無害化します。例えば `|` は `@@PIPE@@` といった、**通常使われることのない文字列に置換**しておきます。処理の最後で復元する必要があるので、無害化する文字が複数ある場合は区別できるようしておかなければなりません。
