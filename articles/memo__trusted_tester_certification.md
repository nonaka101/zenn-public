...
Trusted Tester には他にも、ソフトウェアやドキュメント、モバイルアプリといった分野もありますが、ここで取り扱う資格は[最初](#はじめに)で示した "*DHS Trusted Tester for Web on Windows*" とあるように、**Web コンテンツを対象としたもの**となります。
...
試験問題は 100 問あり、合格するには 85% 以上の正答率が必要です。
...
試験結果は**得点と合否判定のみ**表示されます。また**試験できる回数は 3 回までという制限**があります（制限に達した場合はヘルプデスクに連絡する必要があるとか）。
...
試験の性質上ほぼ必須となるのは、**Windows のデスクトップ環境**です。個人的に必要と感じたのは、翻訳ソフトを挟みながら進めていくための **2 画面構成**です。
...
ただでさえ英語を翻訳しながらという負荷を抱えている都合上、画面は見やすいほうが負担が少ないですし、キー操作も多用するので大きなキーボードがあったほうが楽になります。
...
「ブラウザの翻訳機能を使って、直接翻訳すればいいのでは？」と思われるかもですが、**テストページを直接翻訳するとコンテンツが書き換わり正確な判定ができなくなる**恐れがあるためオススメしません。このあたりは、[後述の「翻訳精度について」](#翻訳精度について)でも説明します。

英語スキルがあって翻訳ソフトが不要な方でしたら、1画面でも問題ないと思います。  
（試験時に使う[最新のテストプロセス](https://github.com/Section508Coordinators/TrustedTester5.1)を PC 上で閲覧する場合は、2画面の方が楽だとは思いますが）

### 英語スキルは必須？

基本的に翻訳ソフトに頼って進められるのですが、個人的な感想としては、少なくとも**高校レベル**はあったほうがいいと感じました。

私の場合、全体の 9 割方は翻訳ソフトを通して学習や試験を行いました。*言い換えれば、残る 1 割の部分については翻訳ソフトを使わず英語のままで進めた* ことになります。どういった場面でそうしたのかについて、ここで説明をしていきます。
...
例えば、講習内でよく出てくる単語の 1 つに "Standard" というものがあります。講習内においては「基準」と訳しておけば大体意味が通るのですが、翻訳ソフトに渡す文脈によっては「標準」だったり「規格」と訳されることがあります。

これら「基準」「標準」「規格」といった表現のブレを こちら側で「あぁ、ここはこういうことを言ってるんだな」と斟酌してあげる必要があります。

##### 文章の省略

試験問題を訳す際、解く側の視点としては「問題文を正確に訳してほしい」というのがあります。ですが翻訳ソフト側は選択肢に繰り返しの表現があった場合、意図的に一部を省略してしまう場面が何度か見られました。
...
またテストプロセスの中にはメディア（音声や映像、同期メディア[^5]など）に関するテストも含まれ、そこでは下記の事象を判断しなければなりません。

[^5]: 同期メディア：音声と映像を組み合わせたメディアのことで、動画サイト等で掲載されている一般的な作品をイメージしてもらえればよい。映像に**合わせて** BGM や音声が流れるが、それを「映像と音声が同期している」と表現する。
...
字幕自体は表示されるので、やろうと思えば手打ちで文字起こしして翻訳ソフトに通すこともできるでしょう。また、スマートフォンのマイクを使って翻訳ソフトに渡す方法もアリかもしれません。
...
試験ではテストページを使って実際にテストしていくことになりますが、このテストページ自体を ブラウザ等に搭載されている翻訳機能を使って翻訳することは推奨しません。
...
私の場合は翻訳ソフトを挟みつつという状況がまずあるので、問題文と訳文で変なことになっていないかの確認も必要になってきます。テストプロセスは訳したものを印刷して手元に用意してたので、参照するのには苦労はしませんでした。
